 ...
 */
function getTicketNameFromSelect(selectEl, doc) {
  try {
    ...
    let ticketNameEl = ticketNameResult.singleNodeValue;
 ...
 * @param {string} [ticketTypeKeyword] 可選的票種關鍵詞；有值則優先選擇名稱含有該關鍵詞的票種，無值則依序：全票 → 單人套票 → 第一個
 * @returns {{hoCode: string, priceCode: string}|null} HoCode 和 PriceCode，如果找不到則返回 null
 */
export function extractHoCodeAndPriceCode(htmlText, logs = [], ticketTypeKeyword) {
  try {
 ...
 */
export async function getHoCodeAndPriceCode(cinemacode, txtSessionId, ticketTypeKeyword) {
