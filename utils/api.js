...
/**
 * 構建訂票頁面 URL（用於抓取 HoCode 和 PriceCode）
 * @param {string} cinemacode 影城代碼（只取數字部分）
 * @param {string} txtSessionId 場次 ID
 * @returns {string} 訂票頁面 URL
 */
export function buildTicketPageUrl(cinemacode, txtSessionId) {
  return `https://sales.vscinemas.com.tw/VieShowTicketT2/?agree=on&cinemacode=${cinemacode}&txtSessionId=${txtSessionId}`;
}
 ...
 */
export async function fetchTicketPage(cinemacode, txtSessionId) {
  try {
    // 取得 cookie
    let cookieHeader = '';
    try {
      ...
      console.warn('取得 cookie 失敗，繼續抓取頁面流程:', error);
    }
    
    // 構建 URL
    const url = buildTicketPageUrl(cinemacode, txtSessionId);
    
    // 構建 Referer URL
    const refererUrl = buildRefererUrl(cinemacode, txtSessionId);
 ...
 * @param {Array<string>} logs 用於收集 log 訊息的陣列
 * @returns {{hoCode: string, priceCode: string}|null} HoCode 和 PriceCode，如果找不到則返回 null
 */
export function extractHoCodeAndPriceCode(htmlText, logs = []) {
  try {
    ...
    // 輔助函數：檢查票種名稱是否為「全票」
    const checkTicketTypeIsFull = (selectElement) => {
      try {
        // 找到 select 元素所在的 tr
        ...
        if (!tr) {
          ...
          tr = parent && parent.tagName === 'TR' ? parent : null;
        }
        ...
        let ticketNameElement = ticketNameResult.singleNodeValue;
    ...
    const candidateElement = result.singleNodeValue;
      ...
      for (let i = 0; i < tables.length; i++) {
        ...
        if (firstRow) {
          ...
          if (cells.length >= 3) {
            ...
            if (select && checkTicketTypeIsFull(select)) {
 ...
 * @param {string} cinemacode 影城代碼（只取數字部分）
 * @param {string} txtSessionId 場次 ID
 * @returns {Promise<{hoCode: string, priceCode: string, logs: string[]}>} HoCode、PriceCode 和 log 訊息
 */
export async function getHoCodeAndPriceCode(cinemacode, txtSessionId) {
  ...
  try {
    ...
    if (result && result.hoCode && result.priceCode) {
      ...
      return {
        hoCode: result.hoCode,
        priceCode: result.priceCode,
        logs
      };
    } else {
      logs.push('無法從網頁取得 HoCode 和 PriceCode，使用預設值');
      return {
        hoCode: DEFAULT_HO_CODE,
        priceCode: DEFAULT_PRICE_CODE,
        logs
      };
    }
  } catch (error) {
    logs.push(`取得 HoCode 和 PriceCode 失敗: ${error.message || error}`);
    logs.push('使用預設值');
    return {
      hoCode: DEFAULT_HO_CODE,
      priceCode: DEFAULT_PRICE_CODE,
 ...
 * @param {Object} params Checkout 參數
 * @param {string} params.cinemaCode 影城代碼（從選單值提取數字部分）
 * @param {string} params.sessionId 場次 ID（從時間選單的值取得）
 * @returns {Promise<Response>} API 回應
