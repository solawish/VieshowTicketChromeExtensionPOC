 ...
 * @returns 訂票頁面 URL
 */
export function buildTicketPageUrl(cinemacode: string, txtSessionId: string): string {
 ...
 * @returns HTML 內容
 */
export async function fetchTicketPage(cinemacode: string, txtSessionId: string): Promise<string> {
  try {
    // 取得 cookie
    let cookieHeader = '';
    try {
...
export async function getHoCodeAndPriceCode(cinemacode: string, txtSessionId: string): Promise<{ hoCode: string; priceCode: string; logs: string[] }> {
  ...
  try {
    ...
    logs.push(`取得 HoCode 和 PriceCode 失敗: ${errorMsg}`);
    logs.push('使用預設值');
    return {
      hoCode: DEFAULT_HO_CODE,
      priceCode: DEFAULT_PRICE_CODE,
...
export interface OrderParams {
  ...
  hoCode: string;          // 片名代碼（從網頁動態取得）
  priceCode: string;       // 價格代碼（從網頁動態取得）
 ...
 * 座位預訂參數介面
 */
export interface ReserveSeatsParams {
  cinemaCode: string;  // 影城代碼（從選單值提取數字部分）
  sessionId: string;   // 場次 ID（從時間選單的值取得）
  seats: SeatCoordinate[];  // 座位座標陣列
}
 ...
 * @returns API 回應
 */
export async function reserveSeats(params: ReserveSeatsParams): Promise<Response> {
  try {
    // 如果沒有座位，不呼叫 API
